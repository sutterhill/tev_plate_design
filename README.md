...
Ridgey candidates additionally must score strictly above the AF2-folded TEVd parent for both Ridgey 600M stability and solubility on their own predicted structures. Final designs are chosen by deterministic greedy max-min sequence diversity. ProteinMPNN likelihoods are reported as geometric-mean per-residue probabilities, `exp(-mean NLL)`, averaged over 16 random decoding orders; these are not raw joint probabilities.
